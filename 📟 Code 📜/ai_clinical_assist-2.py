...
# =========================
    ...
    st.header("📝 Full Health Report Generator (Prescription + Lab Reports)")
    uploaded_files = st.file_uploader(
        "Upload multiple files (Prescriptions / Lab Reports / PDFs / Images) 🗂️", 
    ...
    if uploaded_files:
        ...
        for file_index, file in enumerate(uploaded_files, start=1):
            st.subheader(f"File {file_index}: {file.name}")
            try:
                if file.type == "application/pdf":
                    pages = convert_from_bytes(file.read())
                    st.success(f"PDF has {len(pages)} page(s)")
                    for i, page in enumerate(pages, start=1):
                        page_text = pytesseract.image_to_string(page)
                        combined_text += f"\n\n--- {file.name} | Page {i} ---\n{page_text}"
                        st.text_area(f"{file.name} - Page {i} OCR", page_text, height=150)
                else:
                    image = Image.open(file)
                    page_text = pytesseract.image_to_string(image)
                    combined_text += f"\n\n--- {file.name} ---\n{page_text}"
                    st.text_area(f"{file.name} OCR", page_text, height=150)
            except Exception as e:
        ...
        if combined_text:
            ...
            Here are multiple patient documents (prescriptions, lab reports, images, PDFs): 
            {combined_text}

            Provide a comprehensive health summary including:
            1. Key findings from each report
            2. Abnormalities or red flags
            3. Suggested next steps or follow-up
            4. Medication or treatment advice
            """
            with st.spinner("Generating full health report... 📝"):
                full_report = get_ai_response(prompt=prompt)
...
# streamlit run ai_clinical_assist-2.py